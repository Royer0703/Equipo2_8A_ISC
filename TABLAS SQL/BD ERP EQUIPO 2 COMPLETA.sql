...
select ROW_NUMBER() over(order by idCliente asc)idCliente,nombre,razonSocial,limiteCredito,rfc,telefono,email,tipo from SalesClientes
...
select ROW_NUMBER() over(order by idDireccion asc)idDireccion,calle,numero,colonia,codigoPostal,tipo,idCliente,idCiudad from SalesDireccionesCliente
